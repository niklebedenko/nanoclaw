...
async function runScript(script: string, args: object): Promise<SkillResult> {
  const scriptPath = path.join(
    process.cwd(), '.claude', 'skills', 'ocado-integration', 'scripts', `${script}.ts`
  );
  ...
  return new Promise((resolve) => {
    ...
    proc.on('close', (code) => {
      clearTimeout(timer);
      if (stderr) logger.debug({ script, stderr: stderr.slice(0, 500) }, 'Script stderr');
      if (code !== 0) {
...
export async function handleOcadoIpc(
  ...
  if (!isMain) {
    logger.warn({ sourceGroup, type }, 'Ocado integration blocked: not main group');
    return true;
  }
  ...
  switch (type) {
    ...
    case 'ocado_add_to_basket':
      if (!data.items) {
        result = { success: false, message: 'Missing items array' };
        break;
      }
      result = await runScript('add-to-basket', { items: data.items });
      break;
  ...
  if (result.success) {
    ...
    logger.error({ type, requestId, message: result.message }, 'Ocado request failed');
  }
  return true;
}
